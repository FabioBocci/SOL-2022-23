...
int read_file_calculate_sum(char* pathToFile) {
    ...
    long sum = 0, line_number = 0;
    char line[256];
    while (fgets(line, sizeof(line), fp)) {
        long num = atol(line);
        sum += num * line_number;
        //fprintf(stderr, "file: %s | linechar: %s | line: %ld | num: %ld | num * line: %ld | sum: %ld \n", pathToFile, line, line_number, num, num * line_number, sum);
        line_number++;
    }
    fclose(fp);
    return sum;
                ...
                while ((entry = readdir(currentDir)) != NULL) {
                    if (entry->d_type == DT_REG) {
                        if (end_with_dat_string(entry->d_name) == 0) {
                            fprintf(stdout, "[mw] Entry was DAT file: %s \n ", entry->d_name);
